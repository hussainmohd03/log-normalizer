...
import { buildCorrectedOcsf, buildRawLog, buildSLMResponse } from 'test/factories'
...
describe('ReviewService', () => {
  ...
  it('queue(): Error propagates (BLOCKING - no try/catch)', async () => {
    ...
    await expect(reviewService.queue(fakeRawLog, slmResponse, PRIORITY.NORMAL)).rejects.toThrow()
  })
  ...
  it('submitCorrection(): Successful correction - validates, updates ManualReview, upserts OCSFEvent', async () => {
    ...
    const ocsfEvent = await prisma.oCSFEvent.findFirst({ where: { rawLogId: rawLog.id } })
  ...
  it('submitCorrection(): Invalid OCSF - throws BadRequestException', async () => {
    ...
    await expect(reviewService.submitCorrection(review.id, correctedOcsf, 'Hussain')).rejects.toThrow(BadRequestException)
  })
  ...
  it('submitCorrection(): Review not found - throws NotFoundException', async () => {
    ...
    await expect(reviewService.submitCorrection('non-existent-id', correctedOcsf, 'Hussain')).rejects.toThrow(NotFoundException)
  })
  ...
  async function createPendingReview(prisma: PrismaService, reviewService: ReviewService) {
    ...
    const slmResponse = buildSLMResponse({ decision: 'review', confidence: 0.65 })
    await reviewService.queue(rawLog, slmResponse, PRIORITY.NORMAL)
    const review = await prisma.manualReview.findFirst({ where: { rawLogId: rawLog.id } })
